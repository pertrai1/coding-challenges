...
3. Create `README.md` with problem statement from LeetCode
...
2. An expert reviewer agent fills out the post-mortem using the `review-solution` skill:
   ...
   - Self-rating rubric
3. Run quality checks: `npx eslint {impl-file}` and `npx prettier --check {impl-file}`

### Phase 4: PULL REQUEST

1. Stage all new files: `git add leetcode/{difficulty}/{number}-{name}/`
